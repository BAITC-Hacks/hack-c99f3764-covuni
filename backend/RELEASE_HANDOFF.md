...
- Для демо жюри включите `AUTH_ENABLED=true` и `DEMO_ACCOUNTS_ENABLED=true`. При старте backend автоматически создаются два аккаунта, если их ещё нет.
...
Демо-аккаунты создаются автоматически при старте, если в `.env` включён `DEMO_ACCOUNTS_ENABLED=true`:
...
Для другой среды значения можно заменить переменными `DEMO_*`. Существующий пользователь не перезаписывается.

Ручной способ остаётся доступным, если автоматическое создание отключено:
